class SimpleDataTableWidget{
    ...
    constructor(containerID)
    {
        ...
        this.contactRef = this.db.ref('contact');
        ...
        this.contactRef.on('child_added', function(data) {
            let newContact = data.val();
            newContact.key = data.key;
            thisObj.allContacts.push(newContact);
            thisObj.refresh();
        });

        this.contactRef.on('child_changed', function(data) {
            let newContact = data.val();
            newContact.key = data.key;
            let foundIndex = thisObj.allContacts.findIndex(function (item) {
                return item.key === data.key;
            });
            if(foundIndex !== -1)
            {
                thisObj.allContacts[foundIndex] = newContact;
            }
            thisObj.refresh();
        });

        this.contactRef.on('child_removed', function(data) {
            thisObj.allContacts = thisObj.allContacts.filter(function(item){
                return item.key !== data.key;
            });
            thisObj.refresh();
        });
    ...
    createContact(owner, type, year_of_add, value) {
        ...
        this.db.ref('contact').push(newUser);
    }
    ...
    updateContact(key,owner, type, year_of_add, value)
    {
        ...
        this.db.ref('contact/'+key).set(newUser);
    }


    deleteContact(key)
    {
        this.db.ref('contact/'+key).remove();
    }
    ...
    renderWidget()
    {
        ...
        controlPanel.append($('<button class="btn btn-secondary" style=" margin-left: 10px ;">').text('Refresh').click(function (objName) {window[objName].refresh();}));
        ...
        {
            let rowHtml = $('<tr>');
            rowHtml.append($('<td>').html('<input type="checkbox" name="type" value="'+this.displayContacts[i].key+'" />'));
            ...
            rowHtml.append( $('<td id="key' + i + '" hidden>').text(this.displayContacts[i].key));
...
function onClickUpdateContact(event) {
    ...
    datatableonj.updateInputKey.val(datatableonj.displayContacts[index].key);
}
